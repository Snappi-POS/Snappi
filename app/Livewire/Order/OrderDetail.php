...
class OrderDetail extends Component
{
        ...
        if ($status == 'billed') {
            ...
            foreach ($this->order->kot as $kot) {
                foreach ($kot->items as $item) {
                    ...
                    if (!$this->inventory->canFulfillSale($menuItem, $quantity)) {
    ...
    public function cancelOrder($id)
    {
        $order = Order::with('orderItems.menuItem')->find($id); // eager-load items + menuItems

        if ($order) {
            // 1️⃣ Restore quantities to stock
            $inventory = app(\App\Services\InventoryService::class);

            foreach ($order->orderItems as $orderItem) {
                if ($orderItem->menuItem) {
                    $inventory->rollbackSale(
                        $orderItem->menuItem,
                        $orderItem->quantity,  
                        $order->id
                    );
            ...
            // 2️⃣ Update order status and related models
            $order->update([
                'status' => 'canceled',
            ]);
