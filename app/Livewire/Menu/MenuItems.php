...
class MenuItems extends Component
{
    ...
    public function render()
    {
        ...
        // Query setup — eager-load stock and related tables
        $query = MenuItem::withoutGlobalScope(AvailableMenuItemScope::class)
            ->with(['category', 'menu', 'stock', 'variations']) // 👈 include stock relation
        ...
        // Search and paginate
        $menuItems = $query->search('item_name', $this->search)
            ->orderBy('id', 'desc')
            ->paginate(10);
        ...
        foreach ($menuItems as $item) {
             if ($item->variations_count > 0) {
                // Sum quantities from variations
                $item->current_quantity = $item->variations->sum('quantity');
            } else {
                // Use main stock
                $item->current_quantity = $item->stock ? $item->stock->quantity : 0;
            }
        }
