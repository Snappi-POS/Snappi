...
class Cart extends Component
{
    ...
    public function placeOrder($pay = false, $updateOrder = null, $method = null)
    {
        if ($updateOrder) {
            ...
            // $path = $this->image ? $this->image->store('receipts', 'public') : null;
            $path = $this->image ? $this->image->storeAs('receipts', time() . '.' . $this->image->getClientOriginalExtension(), 'custom_public') : null;
    ...
    public function render()
    {
        ...
        if (!empty($this->search)) {
            $keywords = explode(' ', $this->search); // split by spaces
