...
class AddMenuItem extends Component
{
    ...
    public $quantity_in_measure = 1;
    public $isSubMeasure = false;
    ...
    public function mount()
    {
        ...
        // $this->measurements = Measurement::where('branch_id', branch()->id)->get();
        // $this->measurements = Measurement::all();
    }

    public function updateQuantityField()
        {
            $measurement = Measurement::find($this->measurement_id);

            if ($measurement && $measurement->submeasure) {
                $this->isSubMeasure = true;
                $this->quantity_in_measure = null; // let user fill
            } else {
                $this->isSubMeasure = false;
                $this->quantity_in_measure = 1; // lock to 1
            }
        }
    ...
    public function submitForm()
    {
        ...
        $menuItem = MenuItem::create([
            ...
            'measurement_id' =>  null,
            'quantity_in_measure' => 1,
            'barcode' => (!$this->hasVariations) ? $this->barcode : null,
        ]);

        $inventory->addNewStock($menuItem, $this->initial_quantity);
            ...
            foreach ($this->variationName as $key => $value) {
                if (!empty($value)) {
                    ...
                    MenuItemVariation::create([
                        ...
                        'barcode' => $this->variationBarcode[$key],
                        'quantity' => $this->variationQuantity[$key]
                    ]);
                }
            }
        }
