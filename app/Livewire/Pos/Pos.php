...
class Pos extends Component
{
    ...
    public  $barcodeError = null;
    public  $barcodeSuccess = null;
    ...
    public function mount()
    {
        ...
        // Handle redirect from another page with ?scan=barcode
        if ($barcode = request('scan')) {
    ...
    //     $product = MenuItem::withCount('variations', 'modifierGroups')
    //         ->where('barcode', $barcode)
    //         ->first();
    
    //     if (!$product) {
    //         $this->alert('error', __('messages.barcodeNotFound'), [
    //         'toast' => true,
    //         'position' => 'top-end',
    ...
    //     $this->barcodeSuccess = "Added: {$product->name}";
    // }
    ...
    public function addByBarcode(string $barcode): void
{
    ...
    // First, try to find the barcode in menu_items table
    $product = MenuItem::withCount('variations', 'modifierGroups')
        ->where('barcode', $barcode)
        ->first();
    ...
    // If not found in menu_items, search in menu_item_variations table
    $variation = MenuItemVariation::where('barcode', $barcode)->first();
    ...
    // Handle variation the same way setPosVariation does
    $modifiersAvailable = $variation->menuItem->modifiers->count();

    if ($modifiersAvailable) {
        // Has modifiers — open the modifiers modal
        $this->selectedModifierItem = $variation->menu_item_id . '_' . $variation->id;
        $this->showModifiersModal = true;
    } else {
        // No modifiers — add directly to cart
        $cartKey = '"' . $variation->menu_item_id . '_' . $variation->id . '"';
        $this->orderItemVariation[$cartKey] = $variation;
        $this->syncCart($cartKey);
    }

    $this->barcodeSuccess = "Added: {$variation->variation}";
}
    ...
    public function addCartItems($id, $variationCount, $modifierCount, $qty = null)
    {
    ...
    public function saveOrder($action, $secondAction = null, $thirdAction = null)
    {
        ...
        if ($status == 'billed') {
            ...
            foreach ($this->orderItemList as $key => $value) {
                ...
                $menuItem = isset($this->orderItemVariation[$key])
                    ? $this->orderItemVariation[$key]->menuItem
                    : $this->orderItemList[$key];

                $quantity = $this->orderItemQty[$key];

                if (!$inventory->canFulfillSale($menuItem, $quantity)) {
                    $this->alert('error', __('messages.insufficientStock', [
                        'item' => $menuItem->item_name,
                        'available' => $menuItem->formatted_quantity
                    ]));
                    $sufficient = 0;
                    return;
                }
            ...
            if ($sufficient == 1) {
                ...
                foreach ($this->orderItemList as $key => $value) {
                    ...
                    $this->itemModifiersSelected[$key] = $this->itemModifiersSelected[$key] ?? [];
                    $orderItem->modifierOptions()->sync($this->itemModifiersSelected[$key]);
                    $inventory->deductSale($menuItem, $quantity, $order->id);
                }
