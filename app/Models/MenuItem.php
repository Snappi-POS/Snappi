...
class MenuItem extends BaseModel
{
    ...
    {
        return $this->belongsTo(Measurement::class, 'mainmeasure_id');
    }

    public function submeasure()
    {
        return $this->belongsTo(Measurement::class, 'submeasure_id');
    }
    ...
    public function getFormattedQuantityAttribute()
    {
        ...
        $main = $this->measurement->mainmeasure;
        $sub = $this->measurement->submeasure;
        $perMain = $this->quantity_in_measure;

        // If submeasure exists and quantity_in_measure > 1
        if ($sub && $perMain > 1) {
            $mainCount = intdiv($quantity, $perMain); // whole mains
            $subCount = $quantity % $perMain;         // remainder in subs

            $result = [];

            if ($mainCount > 0) {
                $result[] = $mainCount . ' ' . Str::plural($main, $mainCount);
            }
            if ($subCount > 0) {
                $result[] = $subCount . ' ' . Str::plural($sub, $subCount);
            }

            return implode(', ', $result);
        }

        // If no submeasure → just mainmeasure
