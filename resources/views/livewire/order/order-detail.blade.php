                            ...
                            @if ($order->customer_id)
                                <div class="flex items-center gap-2">
                                    ...
                                    @if(user_can('Update Order'))
                                        <button wire:click="$dispatch('showAddCustomerModal', { id: {{ $order->id }}, customerId: {{ $order->customer_id }} })"
                ...
                @if ($orderProgressStatus === 'cancelled')
                    <div class="p-4 my-4 border border-red-200 rounded-lg bg-red-50 dark:bg-red-900/20 dark:border-red-800">
                        <div class="flex items-start gap-3">
                            <svg class="flex-shrink-0 mt-0.5 w-5 h-5 text-red-500" fill="none" stroke="currentColor"
                                 viewBox="0 0 24 24">
                                    ...
                                    <div class="flex justify-end items-center mt-4 space-x-2 rtl:!space-x-reverse">
                                        ...
                                        @if($currentIndex < count($statuses) - 1)
                                            <x-secondary-button class="inline-flex items-center gap-2"
                                                                wire:click="$set('orderProgressStatus', '{{ $statuses[$nextIndex] }}')">
            ...
            @if ($order)
                            ...
                            @foreach ($order->payments as $key => $item)
                                <tr class="hover:bg-gray-100 dark:hover:bg-gray-700">
                                    ...
                                    @if( $order->placed_via == "shop")
                                        ...
                                        <td class="p-2 text-base text-gray-900 whitespace-nowrap dark:text-gray-400">
                                            @if ($item->receipt)
                                                <a href="{{ asset('receipts/' . $item->receipt) }}" target="_blank"><img
                                                            src="{{ asset('receipts/' . $item->receipt) }} "
                                                            alt="Receipt"
                                                            class="w-16 h-16 object-cover rounded-md mx-auto"></a>
                                            @else
                                    ...
                                    <td
                                        ...
                                        @elseif ($order->status == 'pending_verification' && user_can('Update Order'))
                                            <x-secondary-button class="me-1"
                                                                wire:click="paymentReceived({{ $order->id }}, 'received')">
                ...
                @if ($order->order_type == 'delivery' && $order->delivery_address)
                    <div class="p-3 mt-3 rounded-lg bg-gray-50 dark:bg-gray-700">
                        ...
                        <div class="p-2 text-sm text-gray-600 bg-white border border-gray-200 rounded dark:text-gray-300 dark:bg-gray-800 dark:border-gray-600">
                            <b>Note</b>: {{$order->kot()->where('branch_id', $order->branch_id)->first()?->note ?? '--'}}
                        </div>
    ...
    @if ($order)
        ...
        <x-confirmation-modal wire:model="confirmDeleteModal">
            ...
            <x-slot name="content">
                <div class="flex flex-col w-full space-y-6">
                    ...
                    <div class="p-4 border bg-amber-50 rounded-xl border-amber-200 dark:bg-amber-900/20 dark:border-amber-800">
                        <div class="flex items-start gap-3">
                            <svg class="flex-shrink-0 mt-0.5 w-5 h-5 text-amber-500" fill="none" stroke="currentColor"
                                 viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <x-slot name="footer">
                ...
                <x-danger-button class="ml-3" wire:click="cancelOrderStatus({{ $order->id }})"
                                 wire:loading.attr="disabled">
                    @lang('modules.order.cancelOrder')
                </x-danger-button>
            </x-slot>
