<div>
    ...
    <div class="text-xl font-bold mb-4 text-gray-700 dark:text-gray-200">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div>
            ...
            {{ $latestSubscription?->currentInvoice ? number_format($latestSubscription->currentInvoice->total, 2) : '0.00' }}
        </div>

        <div class="font-semibold text-gray-700 dark:text-gray-300">{{ __('modules.package.paymentDate') }}</div>
        <div>
            {{ $latestSubscription?->currentInvoice && $latestSubscription->currentInvoice->pay_date 
        ...
        <div>
            {{ $latestSubscription?->currentInvoice && $latestSubscription->currentInvoice->next_pay_date 
                ? $latestSubscription->currentInvoice->next_pay_date->format('d-m-Y') 
                : '--' }}
        </div>
            ...
            <x-select id="billing_cycle" class="mt-1 w-full" wire:model.live="billingCycle">
                ...
                @if($selectedPackage?->package_type === App\Enums\PackageType::LIFETIME)
                    <option value="lifetime">
                        {{ __('modules.package.lifetime') }} - {{ $selectedPackage?->currency?->currency_symbol ?? '' }}{{ $selectedPackage?->price ?? 0 }}
                    </option>
                @endif

                @if($packageMonthlyStatus)
                    <option value="monthly">
                        {{ __('modules.package.monthly') }} - {{ $selectedPackage?->currency?->currency_symbol ?? '' }}{{ $monthlyPrice ?? 0 }}
                    </option>
                @endif

                @if($packageAnnualStatus)
                    <option value="annual">
                        {{ __('modules.package.annual') }} - {{ $selectedPackage?->currency?->currency_symbol ?? '' }}{{ $annualPrice ?? 0 }}
                    </option>
                @endif
            </x-select>
