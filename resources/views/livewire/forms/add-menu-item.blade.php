<div class="overflow-y-auto scroll-touch">
        ...
        <div class="space-y-4">
            ...
            <div class="grid grid-cols-1 gap-4">
                ...
                <input
                    ...
                    <!-- Loading Indicator -->
                    <div wire:loading wire:target="itemImage" class="mt-2">
                    ...
                    @if ($itemImage)
                        <div class="mt-2">
                            <img src="{{ $itemImage->temporaryUrl() }}" class="h-32 w-32 object-cover rounded-lg border border-gray-300 dark:border-gray-600">
                        </div>
                    @endif
            ...
            <div>
                    <x-label for="initialQuantity" :value="__('modules.measurements.initialQuantity')" />
                    <x-input id="initialQuantity" class="block mt-1 w-full" type="number" min="1"  placeholder="{{ __('placeholders.initialQuantity') }}" wire:model="initial_quantity" />
                    <x-input-error for="initialQuantity" class="mt-2" />
    
                </div>
            
            <div>
                    <x-label for="barcode" :value="__('modules.menu.barcode')" />
                    <input
                        id="barcode"
                        type="text"
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                        placeholder="{{ __('placeholders.scanBarcode') }}"
                        wire:model="barcode" wire:keydown.enter.prevent 
                        />
            ...
            <div>
                ...
                <div class="grid grid-cols-2 gap-4 mb-4">
                    ...
                    <div>
                        <x-label for="variationQuantity.{{ $key }}" :value="__('modules.measurements.qty')" />
                        <div class="block mt-1 w-full">
                            
                            <x-input id="variationQuantity.{{ $key }}" type="number" min="0"
                                wire:model.live="variationQuantity.{{ $key }}"
                                class="block w-full rounded  text-gray-900 placeholder:text-gray-400"
                                 />
                           
                        </div>
                        <x-input-error for="variationQuantity.{{ $key }}" class="mt-2" />
                    </div>
                    
                    
                    <div>
                        <x-label for="variationBarcode.{{ $key }}" :value="__('modules.menu.barcode')" />
                        <div class="relative rounded-md mt-1 inline-flex items-center">
                            
                            <x-input id="variationBarcode.{{ $key }}" type="text" wire:keydown.enter.prevent 
                                wire:model.live="variationBarcode.{{ $key }}"
                                class="block w-full rounded text-gray-900 placeholder:text-gray-400"
                                 />
                                  <x-danger-button class="ml-2" wire:click="removeField({{ $key }})"
                                wire:key='remove-variation-{{ $key.rand() }}'>&cross;</x-danger-button>
                        </div>
                        <x-input-error for="variationBarcode.{{ $key }}" class="mt-2" />
                    </div>
