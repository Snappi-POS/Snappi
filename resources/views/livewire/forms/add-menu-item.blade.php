    ...
    <form wire:submit="submitForm">
        ...
        <div class="space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div class="mb-4">
                    <x-label for="measurement" :value="__('modules.measurements.measurement')" />
                    <x-select id="measurement_id" class="mt-1 block w-full" wire:model="measurement_id" wire:change="updateQuantityField">
                        <option value="">-- Select Measurement --</option>
                        @foreach($measurements as $measurement)
                        <option value="{{ $measurement->id }}">
                            {{ $measurement->mainmeasure }}
                            @if($measurement->submeasure)
                            / {{ $measurement->submeasure }}
                            @endif
                        </option>
                        @endforeach
                    </x-select>
                ...
                <!-- Quantity in Measure -->
                <div class="mb-4">
                    <x-label for="quantity_in_measure" :value="__('modules.measurements.quantityInMeasure')" />
                    <input
                        id="quantity_in_measure"
                        type="number"
                        min="1"
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                        placeholder="{{ __('placeholders.quantityInMeasure') }}"
                        wire:model="quantity_in_measure"
                        @if(!$isSubMeasure) readonly @endif />
                    <x-input-error for="quantity_in_measure" class="mt-2" />
                </div>
