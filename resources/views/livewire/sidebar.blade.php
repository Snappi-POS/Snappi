    ...
    <aside id="sidebar"
        ...
        <div
            ...
            dark:hover:[&::-webkit-scrollbar-thumb]:bg-gray-400">
                <div
                    ...
                    <ul class="py-2 space-y-2">
                        ...
                        @if ($this->hasModule('Menu') || $this->hasModule('Menu Item') || $this->hasModule('Measurement') || $this->hasModule('Item Category'))
                            @if (user_can('Show Menu') || user_can('Show Menu Item') || user_can('Show Item Category'))
                                <x-sidebar-dropdown-menu :name='__("menu.menuStock")' icon='menu'
                                                         :active='request()->routeIs(["menus.*", "menu-items.*", "item-categories.*", "item-modifiers.*", "modifier-groups.*",  "measurement.*",  "purchases.*", "stock-movements.*"])'>
                                    ...
                                    @if($this->hasModule('Item Category'))
                                        @if(user_can('Show Item Category'))
                                            ...
                                            @livewire('sidebar-dropdown-menu', ['name' => __('menu.units'),'link' => route('measurement.index'),'active' => request()->routeIs('measurement.*')])
                                        @endif
                                        ...
                                        @livewire('sidebar-dropdown-menu', [ 'name' => __('menu.purchases'),'link' => route('purchases.index'),'active' => request()->routeIs('purchases.*')])
                                        @livewire('sidebar-dropdown-menu', [ 'name' => __('menu.stockmovements'),'link' => route('stock-movements.index'),'active' => request()->routeIs('stock-movements.*')])
            ...
            <div class="absolute bottom-0 left-0 justify-center w-full p-2 space-x-4 bg-white md:flex dark:bg-gray-800 rtl:space-x-reverse"
                 sidebar-bottom-menu>
                <a href="{{ module_enabled('Subdomain') ? 'https://'.restaurant()->sub_domain : route('business_page', [restaurant()->hash]) }}"
                   target="_blank"
