            ...
            <div class="mt-4">
                <ul class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-8 gap-3">
                    @forelse ($menuItems as $item)
                        ...
                        <li class="group relative">
                            <input type="checkbox" id="item-{{ $item->id }}" value="{{ $item->id }}"
                                wire:click='addCartItems({{ $item->id }}, {{ $item->variations_count }}, {{ $item->modifier_groups_count }}, {{ $item->quantity }})'
                    ...
                    >
                                ...
                                {{-- Loading Overlay --}}
                                <div wire:loading.flex wire:target="addCartItems({{ $item->id }}, {{ $item->variations_count }}, {{ $item->modifier_groups_count }}, {{ $item->quantity }})"
                                ...
                                <div class="relative aspect-square hidden md:block">
                                    ...
                                    <span class="absolute top-1 right-1 bg-white/90 dark:bg-gray-800/90 rounded-full p-1 shadow-sm">
                                        <span style="font-size:11px"><b>{{ $item->quantity }}</b></span>
                                    </span>
                                </div>
...
<script>
    function handleImageError(img) {
             img.onerror = null;
            img.src = 'data:image/svg+xml;base64,';
    }
</script>